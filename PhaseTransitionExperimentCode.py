...
def run_experiment(bases = [2*  2.63815853], pops = [.1]):
                ...
                for part in exp_chain:
                    ...
                    if total_waits > subsequence_timer + subsequence_step_size:
                        ...
                        if ends:
                            ...
                            if ends_vectors_normalized:
                                #We choose the vector that preserves continuity
                                previous = ends_vectors_normalized[-1]
                        ...
                        while subsequence_timer < total_waits:
                            ...
                            ends_vectors_normalized.append(continuous_lift)
                            balances.append(left_bal)
                            balances.append(right_bal)
                    ...
                    if t % 10000 == 0:
                        ...
                        plt.title("Balances")
                        plt.hist(balances)
                        plt.savefig("./plots/"+str(alignment)+"B"+str(int(100*base))+"P"+str(int(100*pop1))+identifier_string + "balances.png")
                        plt.close()
                        ...
                        max_time = len(ends_vectors_normalized)
                        #plt.title("Angle_Safe")
                        ax.plot ( [ np.arctan2(x[0], x[1]) for x in ends_vectors_normalized] , range(len(ends_vectors_normalized)) ) 
                        ...
                        #ax.circle( (0,0), .1)
                        for k in range(11):
                            plt.polar ( np.arange(0, (2 * np.pi), 0.01), [int(max_time/10) * k ] * len( np.arange(0, (2 * np.pi), 0.01)), '-g' )
                        plt.savefig("./plots/"+str(alignment)+"B"+str(int(100*base))+"P"+str(int(100*pop1)) + str("proposals_") + str( max_time * subsequence_step_size ) + "boundary_slope.png", dpi = 1000)
                        plt.close()
                        plt.close(fig)
