...
def is_clockwise(graph, face, average):
    ...
    angles = [transform(float(np.arctan2(graph.node[x]["pos"][0] - average[0], graph.node[x]["pos"][1] - average[1])))
              for x in face]
    first = min(angles)
...
def compute_face_data(graph):
    ...
    graph.graph["unbounded_face"] = unbounded_face
...
def planar_dual(graph):
    graph = compute_rotation_system(graph)
    graph = compute_face_data(graph)
    dual_graph = nx.Graph()
    unbounded_face = graph.graph["unbounded_face"]

    list_of_faces = graph.graph["faces"]
    list_of_faces.add(unbounded_face)
