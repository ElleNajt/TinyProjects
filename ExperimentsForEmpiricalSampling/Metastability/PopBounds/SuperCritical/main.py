        ...
        for alignment in [2]:
            ...
            for part in exp_chain:
                ...
                if t % 100000 == 0:
                    ...
                    plt.savefig("./plots/"+str(alignment)+"B"+str(int(100*base))+"P"+str(int(100*pop1))+identifier_string + "logflips_bar.png")
                    plt.close()
                    ...
                    key_values = list( range( int (max([int(graph.nodes[x]["lognum_flips"]) for x in graph.nodes()] ))))
                    key_positions = { x : (50, 7*x) for x in key_values}
                    #key_positions[] = (50,-1)
                    key_graph = nx.Graph()
                    for i in key_values:
                        if i < len(key_values) - 1:
                            key_graph.add_edge ( i, i+1 )
                    nx.draw (key_graph, key_positions, node_color =key_values, node_size=10*ns,node_shape='s',cmap="jet")
                    
                    key_positions = { x : (45, 7*x) for x in key_values}
                    nx.draw (key_graph, key_positions, node_color =key_values, labels = { x : (str(x) + ":") for x in key_values}, node_size=0, width = 0, node_shape='s',cmap="jet")
                    plt.title("log(Flips+1)")
